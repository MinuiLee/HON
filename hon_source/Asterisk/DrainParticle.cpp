...
DrainParticle::DrainParticle(unsigned int input_amount_, glm::uvec2 const & size_)
	: Particle(input_amount_, size_)
{
	...
	for (unsigned int i = 0; i < m_amount; i++)
	{
		...
		m_offsetData[i].m_color.Blue = 0.0f;
...
void DrainParticle::Update(float const & dt)
{
	m_isActive = false;
	for (auto & offset_ : m_offsetData)
	{
		if (offset_.m_color.Blue >= 5 * STACK_ACCELRATION_MODIFIER)
			offset_.m_switch = false;
		else if (offset_.m_switch)
		{
			m_isActive = true;
			glm::vec2 left_top(view_pos.x - (view_size.x / 2.0f), view_pos.y + (view_size.y / 2.0f));


			left_top.x -= m_transform.m_translation.x;
			left_top.y -= m_transform.m_translation.y;
			

			offset_.m_transform.m_translation.x +=
				(left_top.x - offset_.m_transform.m_translation.x) * offset_.m_color.Blue;
			offset_.m_transform.m_translation.y +=
				(left_top.y - offset_.m_transform.m_translation.y) * offset_.m_color.Blue;


			offset_.m_color.Blue += dt * STACK_ACCELRATION_MODIFIER;
		}
...
void DrainParticle::Draw(BaseWindow * base_window_ptr)
{
	...
	if (!GetShader())
	{
		auto drain_shader = base_window_ptr->GetShaders().GetShader("Drain");
...
void DrainParticle::SetColor(int input_color) 
{
	if (input_color == BLUE)
	{
		color.r = 0.3f;
